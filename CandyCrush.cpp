#include<iostream>//used for I/O stream
...
void Map_Load(char [ROW][COL]);
//used to load the main menu
void Loading_Page(char [ROW][COL]);
void instructions(char[ROW][COL]);
//used to burst the matching candies
void Insert_Steric(char[ROW][COL]);
//used to swap b/w candies
void swapping(char[ROW][COL]);
void rewrite(char[ROW][COL]);
//used for the movement of the cursor
void move_cursor_to_xy(int, int);
void result();

//used to generate the candies(as symbols)
void Candy_Generator(char arr[ROW][COL])
{
	// We used these symbols as candies
	char candies[5]={'#' , '$' ,'@','?','!'};
	srand(time(0));
	int r;
	for(i=0;i<ROW;i++)
...
void Loading_Page(char arr[][COL])
{
	...
	else
	{
		...
		system("pause");
		system("cls");
		Loading_Page(arr); //recursion use here
	}
}
void Map_Load(char arr[ROW][COL])
{
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(hConsole,15);
	cout<<" +";
...
void Insert_Steric(char arr[ROW	][COL])
{
	srand(time(0));
	int r;
	char candies[5] = { '#' ,'$' , '@' , '?' , '!' };
	...
	for (int i = 0; i < 5; i++)  
	{
		for (int j = 0; j < ROW; j++)  //loops for check matchings in horizontal and adding steric horizontally 
		{
			for (int k = 0; k < COL - 2; k++)
			{
				...
				//checking the 3 candies match and adding the score.
		...
		for (int a = 0; a < ROW - 2; a++)  //loops for checking candies in vertical and adding steric horizontally
		{
			for (int b = 0; b < COL ; b++)
			{
		...
		for (int x = 0; x < ROW; x++) // these loops are created for gravity-effect.
		{
			for (int y = 0; y < COL; y++)
			{
				if (arr[x][y] == '*')
...
void swapping(char arr[ROW][COL])
{
	int x=3,y=1,a=x; 
